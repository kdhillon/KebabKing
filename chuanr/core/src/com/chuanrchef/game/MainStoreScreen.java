...
public class MainStoreScreen extends ScreenTemplate {
	...
	public MainStoreScreen(ChuanrC master) {
		this.master = master;
...
//		return;
	...
	public void setupMainTable() {
		...
		mainTable.setSize(ChuanrC.getWidth(), ChuanrC.getHeight());
		
//		mainTable.debugAll();
		
		// add title
		int title_pad_top = ChuanrC.getGlobalY(0.15f);
		int title_width = ChuanrC.getGlobalX(0.4f);
		int title_height = ChuanrC.getGlobalY(0.07f);
		...
		Label label = new Label(text, Assets.generateLabelStyleUILight(32, true));
		label.setColor(FONT_COLOR);
		mainTable.add(label).center().expandY().top();
		mainTable.row();
		
		// add shelf
		int shelf_width = ChuanrC.getGlobalX(0.8f);
		int shelf_height = ChuanrC.getGlobalY(0.6f);
		int shelf_pos_y = ChuanrC.getGlobalY(0.1f);
		...
		shelf.setPosition((ChuanrC.getWidth() - shelf_width)/2, shelf_pos_y);
	...
	public void addBackButton(Button button, Table table) {
		...
		table.add(button).bottom().right().expandX().width(ChuanrC.getGlobalX(BACK_BUTTON_WIDTH)).height(ChuanrC.getGlobalY(BACK_BUTTON_HEIGHT)).padBottom(ChuanrC.getGlobalY(BACK_BOTTOM_PAD)).padTop(backPad);
	}
