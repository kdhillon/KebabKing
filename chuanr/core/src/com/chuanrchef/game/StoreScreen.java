...
public class StoreScreen extends ScreenTemplate {
	...
	public enum TableType {main, food, grill, map, ads, coins, vanity}; // keeps track of which table is currently selected
	...
	public StoreScreen(ChuanrC master) {
		...
		this.tableWidth = ChuanrC.getGlobalX(63.0f / 64);
		this.tableHeight = ChuanrC.getGlobalY(63.0f / 64);

		this.tableWidth = ChuanrC.getWidth();
		this.tableHeight = ChuanrC.getHeight() - (int) (ChuanrC.getGlobalY(DrawUI.UI_BAR_HEIGHT)) + 10; // total size of table, everything in it.
		...
		//		table.setPosition((ChuanrC.getWidth() - tableWidth) / 2, (ChuanrC.getHeight() - tableHeight) / 8);
		table.setPosition((ChuanrC.getWidth() - tableWidth) / 2, 0);

		//		table.setPosition((ChuanrC.getWidth() - tableWidth) / 2, (ChuanrC.getHeight() - tableHeight) / 2);
	...
	public void resetTable(TableType type) {
		...
		case ads:
			...
			break;
		case vanity:
			vanityTable = createTable("Vanity", new PurchaseType[] {master.profile.inventory.decorations, master.profile.inventory.grillStand});
			break;
		case coins:
	...
	public void setupMainTable() {
		...
		Button otherButton = generateMainButton("\"Vanity\"", TableType.vanity, null);
