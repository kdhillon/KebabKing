...
public class StoreScreen extends ScreenTemplate {
	...
	public StoreScreen(ChuanrC master, MainStoreScreen mainStoreScreen) {
		this.master = master;
		...
		this.tableWidth = ChuanrC.getWidth();
		this.tableHeight = ChuanrC.getHeight();
...
//		mainTable.debugAll();
		...
		mainTable.setPosition((ChuanrC.getWidth() - tableWidth) / 2, 0);
...
//		System.out.println("updating purchaseable table with current purchasable index " + currentPurchaseableIndex);
		...
		int buttonHeight = ChuanrC.getGlobalX(0.15f);
...
//			System.out.println("reg Width > regHeight" + regWidth + " , " + regHeight);
		...
		boolean lockedByRound = !type.isUnlockedByRound(purchaseable);
		...
		int infoPad = ChuanrC.getGlobalX(0.03f);
		int infoWidth = mainWidth - infoPad - buttonWidth;
		Label pTitle = new Label(purchaseable.getName(), Assets.generateLabelStyleUILight(24, true));
		pTitle.setColor(color);
		pTitle.setAlignment(Align.left);
		info.add(pTitle).left();
		
		// TODO add daily cost here
		Label pPrice1 = new Label("$" + floatToString(purchaseable.getDailyCost()), Assets.generateLabelStyleUIHeavy(24, true));
		...
		Label pPrice2 = new Label(" / DAY", Assets.generateLabelStyleUI(16, true));
		if (purchaseable.getDailyCost() <= 0) {
			pPrice2.setText(" ");
		}
		pPrice2.setColor(MainStoreScreen.FONT_COLOR_GREEN);
		pPrice2.setAlignment(Align.right);
		info.add(pPrice2).right().bottom().padBottom(ChuanrC.getGlobalY(0.002f));
		...
		if (locked && !lockedByRound) {
			...
			int unlockHeight = ChuanrC.getGlobalY(0.04f);
			Button unlockButton = createUnlockButton(purchaseable, type, unlockHeight);
			info.add(unlockButton).height(unlockHeight).width(ChuanrC.getGlobalX(0.4f)).left().colspan(3);
		}
		// otherwise just draw with gray
		else {
			Label pDesc = new Label(purchaseable.getDescription(), Assets.generateLabelStyleUILight(18, true));
			if (purchaseable.getDescription() == null || purchaseable.getDescription().length() == 0) {
				pDesc = new Label("???", Assets.generateLabelStyleUILight(16, true));
			}
			if (lockedByRound) 
				pDesc.setText("Available after Day " + purchaseable.unlockAtRound());
			pDesc.setWrap(true);
...
//		else {
		...
		Label unlock = new Label("Unlock", Assets.generateLabelStyleUIChina((int) (height * 0.9f), true));
		...
		bothButtons.add(unlockTable);
		
		Table priceTable = new Table();
		Label price = new Label("", Assets.generateLabelStyleUIChina((int) (height * 0.9f), true));
...
//		}
	...
	public Table updateSelectedPurchaseableTable(Purchaseable purchaseable, PurchaseType type) {
		...
		//
		// first add image
		int imageHeight = ChuanrC.getGlobalY(0.1f);
		...
		int iconPad = ChuanrC.getGlobalY(0.05f);
		Image icon = new Image(iconReg); 
		
		table.add(icon).width(imageWidth).height(imageHeight).padTop(iconPad);
		table.row();
		Label title = new Label(purchaseable.getName(), Assets.generateLabelStyleUI(35, false));
		title.setColor(MainStoreScreen.FONT_COLOR);
		table.add(title);
		table.row();
		Label description = new Label(purchaseable.getDescription(), Assets.generateLabelStyleUILight(22, false));
	...
	public Table createTable(String titleString, PurchaseType[] types) {
		...
		Table table = new Table();
//		table.debugAll();
		table.setWidth(ChuanrC.getWidth());
		table.setHeight(ChuanrC.getHeight());

		mainWidth = ChuanrC.getGlobalX(0.8f);

		Table main = new Table();
//		main.debugAll();
		main.setWidth(mainWidth);
		main.setHeight(ChuanrC.getGlobalY(0.7f));
		main.row();
		table.add(main).center().top().expandY();

		// add buttons for each table
		int buttonPad = ChuanrC.getGlobalX(0.01f);
		int buttonWidth = mainWidth / types.length - buttonPad;
		int buttonHeight = ChuanrC.getGlobalY(0.05f);
		...
		main.add(buttonTable).top().expandY().padTop(ChuanrC.getGlobalY(0.1f));
