...
public class TrashPile {
	...
	public void draw(SpriteBatch batch) {
		if (ks.kebabsTrashed >= TRASH_PILE_BIG) {
			batch.draw(Assets.trashIcon, ChuanrC.getGlobalX(TP_X_L), ChuanrC.getGlobalY(TP_Y), ChuanrC.getGlobalX(TP_WIDTH_L), ChuanrC.getGlobalY(TP_HEIGHT_L));
		}
		else if (ks.kebabsTrashed >= TRASH_PILE_MED) {
			batch.draw(Assets.trashIcon, ChuanrC.getGlobalX(TP_X_M), ChuanrC.getGlobalY(TP_Y), ChuanrC.getGlobalX(TP_WIDTH_M), ChuanrC.getGlobalY(TP_HEIGHT_M));			
		}
		else if (ks.kebabsTrashed >= TRASH_PILE_SMALL) {
			batch.draw(Assets.trashIcon, ChuanrC.getGlobalX(TP_X_S), ChuanrC.getGlobalY(TP_Y), ChuanrC.getGlobalX(TP_WIDTH_S), ChuanrC.getGlobalY(TP_HEIGHT_S));
		}
	}
}
