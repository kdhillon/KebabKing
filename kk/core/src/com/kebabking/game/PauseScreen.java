...
public class PauseScreen extends ScreenTemplate  {
	...
	public PauseScreen(KebabKing master) {
		...
		table = new Table();
		table.setPosition(KebabKing.getGlobalX(1.0f / 8), KebabKing.getGlobalY(1.0f / 4));
		table.setSize(KebabKing.getGlobalX(3.0f / 4), KebabKing.getGlobalY(1.0f / 2));
//		table.debugAll();
		table.align(Align.top);
		uiStage.addActor(table);
		
		Label paused = new Label("Paused!", Assets.generateLabelStyleUIChinaWhite(100));
		table.add(paused);
...
//		});	
		...
		table.row();
		table.add(resume).padTop(KebabKing.getGlobalY(1.0f / 32));
